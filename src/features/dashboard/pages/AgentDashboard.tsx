    ...
    <div className="dash">
      <header className="dash-topbar">
        ...
        <div className="dash-topbar-right">
          ...
          />
          <span className="dash-role-badge dash-role-badge--agent">Support Agent</span>
          <span style={{ fontSize: '0.8rem', color: 'var(--slate-400)', cursor: 'pointer' }} onClick={() => navigate('profile')}>{user?.email}</span>
          <div className="dash-avatar" style={{ background: 'linear-gradient(135deg,#34D399,#059669)' }} onClick={() => navigate('profile')} title="Go to Profile">{initials}</div>
