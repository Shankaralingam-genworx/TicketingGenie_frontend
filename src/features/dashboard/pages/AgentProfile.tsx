...
export default function AgentProfile() {
  ...
  useEffect(() => { fetchAssignedTickets().then(setTickets).catch(() => {}); }, []);
  ...
  return (
    <>
      <div className="dash-page-hdr">
        ...
        <p className="dash-page-sub">Your agent account and workload summary.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20, alignItems: 'start' }}>
        ...
        <div style={{ background: 'white', border: '1px solid var(--slate-200)', borderRadius: 14, padding: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, textAlign: 'center' }}>
          <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg,#34D399,#059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.6rem', fontWeight: 800, color: 'white' }}>
            {initials}
          </div>
          <div>
            <p style={{ margin: 0, fontWeight: 700, fontSize: '1rem', color: 'var(--slate-800)' }}>{user?.email}</p>
            <span style={{ background: '#ECFDF5', color: '#065F46', padding: '3px 12px', borderRadius: 99, fontSize: '0.75rem', fontWeight: 700, marginTop: 6, display: 'inline-block' }}>
              Support Agent
            </span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ background: 'white', border: '1px solid var(--slate-200)', borderRadius: 14, padding: 24 }}>
            <h3 style={{ margin: '0 0 16px', fontSize: '0.85rem', color: 'var(--slate-600)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Workload Summary</h3>
          ...
          <div style={{ background: 'white', border: '1px solid var(--slate-200)', borderRadius: 14, padding: 24 }}>
            <h3 style={{ margin: '0 0 14px', fontSize: '0.85rem', color: 'var(--slate-600)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Account Details</h3>
            {[
              ...
              <div key={r.label} style={{ display: 'flex', gap: 12, padding: '9px 0', borderBottom: '1px solid var(--slate-100)' }}>
                <span style={{ minWidth: 120, fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--slate-500)' }}>{r.label}</span>
                <span style={{ color: 'var(--slate-800)', fontSize: '0.88rem' }}>{r.value}</span>
              </div>
            ))}
          </div>
        </div>
