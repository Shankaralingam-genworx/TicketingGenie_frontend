 ...
 *
 * env.API_AUTH_URL already includes /api/v1 — no extra concatenation needed.
 */
...
async function doRefresh(): Promise<string> {
  ...
  refreshPromise = (async () => {
    // env.API_AUTH_URL already includes /api/v1
    const res = await fetch(`${env.API_AUTH_URL}/auth/refresh`, {
      method:      "POST",
      credentials: "include",
    });
